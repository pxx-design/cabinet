...
1. **克隆进入** —— `git clone https://github.com/pxx-design/cabinet && cd cabinet`
2. **用 Claude Code 打开这个文件夹**（或任何能读 `INGEST.md` 的 agent）。
3. **本地起服务** —— 跑 `./start.sh`，在 `http://localhost:8777` 打开。
   磁盘桥必须走 `localhost`——Chrome 在 `file://` 下会拦掉文件夹授权，所以
   双击 `index.html` 只能浏览、不能保存或入库。
4. **连接** —— 点左下「连接」（那颗脉冲红点）→ 选你刚克隆的 `cabinet` 文件夹。
5. **用起来** —— 在感觉地图上逛、按感觉召唤。要加自己的图：粘参考（`⌘V`）→
   点「入库」暂存 → 在 Claude Code 里跑 **`/ingest`** 拆成手法。在任何别的项目
   里跑 **`/translate`**，把你收的手法用进去。
